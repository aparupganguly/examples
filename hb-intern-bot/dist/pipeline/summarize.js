    ...
    try {
        ...
        if (content) {
            try {
                ...
                return {
                    summary: (0, utils_js_1.truncate)(parsed.summary || generateFallbackSummary(event), 200),
                    why_matters: (0, utils_js_1.truncate)(parsed.why_matters || generateFallbackWhyMatters(event), 150)
                };
            }
            catch {
...
function generateFallbackSummary(event) {
    const bullets = [];
    // Title summary
    bullets.push(`• ${(0, utils_js_1.truncate)(event.title, 80)}`);
    // Source info
    ...
    if (event.points > 0 || event.comments > 0) {
        const engagement = [];
        if (event.points > 0)
            engagement.push(`${event.points} points`);
        if (event.comments > 0)
            engagement.push(`${event.comments} comments`);
        bullets.push(`• ${engagement.join(', ')}`);
    }
    return bullets.slice(0, 3).join('\n');
}
function generateFallbackWhyMatters(event) {
...
async function summarizeBatch(events, batchSize = 5) {
    const results = [];
